...
class auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        //cari di DB user yg emailny sama
    ...
    public function registrasi()
    {
        ...
        //trim = ga input spasi ke database
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                ...
                'student_id' => htmlspecialchars($this->input->post('id', true)),
            ...
            //Token
            $token = base64_encode(random_bytes(32)); //Random token
            $user_token = [
                'email' => $email,
                'token' => $token,
                'date_created' => time()
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'bagdrack85@gmail.com',
            'smtp_pass' => 'aapqlmlsfsyxrnvf',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
        ...
        $this->email->from('bagdrack85@gmail.com', 'SIAKAD - Email Verification');
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Click this link to reset you password : <a href="' . base_url() . 'auth/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Pasword</a>');
        }
        if ($this->email->send()) {
            return true;
        } else {
